"""
Complaint spike alert script for AutoRespondX.
Read-only, safe, and demo-friendly.
"""
import os
import sys
import psycopg2
...
def main():
    ...
    try:
        with conn.cursor() as cur:
            ...
            row = cur.fetchone()
            if row is None:
                print("No data found in vw_complaint_spike.")
                sys.exit(0)
            count = row[0]
            print(f"Complaint count in last 20 tweets: {count}")
            if count >= threshold:
